...
class NoiseScheduleVP:
    def __init__(
        ...
        ***
        ...
        if schedule == 'discrete':
            ...
            self.T = 1.
            self.t_array = jt.linspace(0., 1., self.total_N + 1)[1:].reshape((1, -1)).to(dtype=dtype)
    ...
    def logaddexp(a, b):
        max_val = jt.maximum(a, b)
        return max_val + jt.log(jt.exp(a - max_val) + jt.exp(b - max_val))
    ...
    def inverse_lambda(self, lamb):
        ...
        if self.schedule == 'linear':
            tmp = 2. * (self.beta_1 - self.beta_0) * self.logaddexp(-2. * lamb, jt.zeros((1,)).to(lamb))
            Delta = self.beta_0**2 + tmp
            return tmp / (jt.sqrt(Delta) + self.beta_0) / (self.beta_1 - self.beta_0)
        elif self.schedule == 'discrete':
            log_alpha = -0.5 * self.logaddexp(jt.zeros((1,)), -2. * lamb)
            t = interpolate_fn(log_alpha.reshape((-1, 1)), jt.flip(self.log_alpha_array, [1]), jt.flip(self.t_array, [1]))
            return t.reshape((-1,))
        else:
            log_alpha = -0.5 * self.logaddexp(-2. * lamb, jt.zeros((1,)).to(lamb))
            t_fn = lambda log_alpha_t: jt.arccos(jt.exp(log_alpha_t + self.cosine_log_alpha_0)) * 2. * (1. + self.cosine_s) / math.pi - self.cosine_s
            t = t_fn(log_alpha)
            return t
...
def model_wrapper(
    ...
    def cond_grad_fn(x, t_input):
        ...
        with jt.enable_grad():
            ...
            return jt.autograd.grad(log_prob.sum(), x_in)[0]
...
class DPM_Solver:
    ...
    def dynamic_thresholding_fn(self, x0, t):
        ...
        s = expand_dims(jt.maximum(s, self.thresholding_max_val * jt.ones_like(s)), dims)
    ...
    def get_time_steps(self, skip_type, t_T, t_0, N):
        ...
        """
        if skip_type == 'logSNR':
            lambda_T = self.noise_schedule.marginal_lambda(jt.array(t_T))
            lambda_0 = self.noise_schedule.marginal_lambda(jt.array(t_0))
            logSNR_steps = jt.linspace(lambda_T.cpu().item(), lambda_0.cpu().item(), N + 1)
            return self.noise_schedule.inverse_lambda(logSNR_steps)
        elif skip_type == 'time_uniform':
            return jt.linspace(t_T, t_0, N + 1)
        elif skip_type == 'time_quadratic':
            t_order = 2
            t = jt.linspace(t_T**(1. / t_order), t_0**(1. / t_order), N + 1).pow(t_order)
            return t
        else:
    ...
    def get_orders_and_timesteps_for_singlestep_solver(self, steps, order, skip_type, t_T, t_0):
        ...
        if skip_type == 'logSNR':
            ...
            timesteps_outer = self.get_time_steps(skip_type, t_T, t_0, steps)[jt.cumsum(jt.array([0,] + orders), 0)]
    ...
    def singlestep_dpm_solver_second_update(self, x, s, t, r1=0.5, model_s=None, return_intermediate=False, solver_type='dpmsolver'):
        ...
        alpha_s1, alpha_t = jt.exp(log_alpha_s1), jt.exp(log_alpha_t)
    ...
    def singlestep_dpm_solver_third_update(self, x, s, t, r1=1./3., r2=2./3., model_s=None, model_s1=None, return_intermediate=False, solver_type='dpmsolver'):
        ...
        alpha_s1, alpha_s2, alpha_t = jt.exp(log_alpha_s1), jt.exp(log_alpha_s2), jt.exp(log_alpha_t)
    ...
    def dpm_solver_adaptive(self, x, order, t_T, t_0, h_init=0.05, atol=0.0078, rtol=0.05, theta=0.9, t_err=1e-5, solver_type='dpmsolver'):
        ...
        """
        ns = self.noise_schedule
        s = t_T * jt.ones((1,)).to(x)
        lambda_s = ns.marginal_lambda(s)
        lambda_0 = ns.marginal_lambda(t_0 * jt.ones_like(s).to(x))
        h = h_init * jt.ones_like(s).to(x)
        x_prev = x
        nfe = 0
        if order == 2:
            r1 = 0.5
        ...
        while jt.abs((s - t_0)).mean() > t_err:
            ...
            delta = jt.max(jt.ones_like(x).to(x) * atol, rtol * jt.max(jt.abs(x_lower), jt.abs(x_prev)))
            norm_fn = lambda v: jt.sqrt(jt.square(v.reshape((v.shape[0], -1))).mean(dim=-1, keepdim=True))
            ...
            if jt.all(E <= 1.):
                ...
                x_prev = x_lower
                lambda_s = ns.marginal_lambda(s)
            h = jt.min(theta * h * jt.pow(E, -1. / order).float(), lambda_0 - lambda_s)
            nfe += order
    ...
    def add_noise(self, x, t, noise=None):
        ...
        alpha_t, sigma_t = self.noise_schedule.marginal_alpha(t), self.noise_schedule.marginal_std(t)
        if noise is None:
            noise = jt.randn((t.shape[0], *x.shape))
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        with jt.no_grad():
            if method == 'adaptive':
                ...
                x = self.model_fn(x, t)
                x = jt.where(input_ids_mask==0, x_start, x)
...
def interpolate_fn(x, xp, yp):
    ...
    all_x = jt.cat([x.unsqueeze(2), xp.unsqueeze(0).repeat((N, 1, 1))], dim=2)
    sorted_all_x, x_indices = jt.sort(all_x, dim=2)
    x_idx = jt.argmin(x_indices, dim=2)[0]
    # import torch
    # tr = torch.argmin(torch.tensor(x_indices.numpy()), dim=2)
    # print("lkhdbchsjvblhsbv ",x_idx)
    # print("torch"," ",tr)
    ...
    start_idx = jt.where(
        ...
        jt.array(1),
        jt.where(
            (x_idx == K), jt.array(K - 2), cand_start_idx,
        ),
    )
    end_idx = jt.where((start_idx==cand_start_idx), start_idx + 2, start_idx + 1)
