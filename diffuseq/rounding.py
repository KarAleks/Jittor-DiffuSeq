...
def get_knn(model_emb, text_emb, dist='cos'):
    if dist == 'cos':
        ...
        adjacency = -jt.norm(adjacency, dim=-1)
    topk_out = jt.topk(adjacency, k=6, dim=0)
    return topk_out[0], topk_out[1]
...
def compute_logp(args, model, x, input_ids):
    ...
    ce = jt.nn.CrossEntropyLoss(reduction='none')
