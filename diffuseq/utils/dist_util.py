...
def setup_dist():
    ...
    backend =  "mpi" #"gloo" if not th.cuda.is_available() else "nccl"
    ...
    jt.flags.use_cuda = jt.has_cuda  # Enable CUDA if available
    jt.flags.mpi = True  # Enable MPI
    jt.mpi.init()
...
def dev():
    ...
    if jt.has_cuda:
        return jt.device(f"cuda:{os.environ['LOCAL_RANK']}")
