...
def normal_kl(mean1:jt.Var, logvar1:jt.Var, mean2:jt.Var, logvar2:jt.Var):
    """
    ...
    for obj in (mean1, logvar1, mean2, logvar2):
        if isinstance(obj, jt.Var):
            tensor = obj
            break
    assert tensor is not None, "at least one argument must be a Jittor Var"

    logvar1, logvar2 = [
        x if isinstance(x, jt.Var) else jt.Var(x).to(tensor)
